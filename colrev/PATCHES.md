...
| `colrev/dataset.py` | `load_records_dict()` serves a process-wide cache of the parsed records file, keyed on the file's `(st_mtime_ns, st_size)` and handed out as a deep copy; `save_records_dict()` updates the entry instead of dropping it (a partial save merges only the records it names). `invalidate_records_cache()` is exposed for callers that replace `records.bib` behind the Dataset. `header_only=True` is deliberately uncached. | Parsing `records.bib` dominates every records operation (~2.2 s at 5k records) and the JSON-RPC server pays it two or more times per user action, since it builds a fresh `ReviewManager` per request. Callers already received a freshly-built dict from every call, so returning a copy is semantically identical — the cache is invisible except in latency. At 5k records: `get_prescreen_queue` 6.6 s → 0.3 s warm, `prescreen_record` 3.7 s → 1.0 s. |
...
| `colrev/process/status.py` | (WP-06) `currently.md_retrieved` is accounted per source file: `_get_md_retrieved_per_source` returns `{filename: nr_records}`, and `_get_currently_md_retrieved` subtracts each file's imported origins from that file's own count with a floor of zero per file. Upstream computed a global `sum(files) - len(non_md_origins)`, which goes negative when a search file shrinks (API re-run returning fewer records) or a source file is removed after load, and lets one file's deficit corrupt other sources' pending counts. | The app's status payload previously clamped/re-summed these numbers in `response_formatter.py`; that patch-over is gone — UI totals are the engine's numbers. |
