...
const includeReviewers = computed(() =>
  props.reviewers.filter(
    (r) => reviewerStatusToDecision(r.status, props.kind) === 'include',
  ),
);

const excludeReviewers = computed(() =>
  props.reviewers.filter(
    (r) => reviewerStatusToDecision(r.status, props.kind) === 'exclude',
  ),
);
</script>
...
<template>
  <div
    class="flex items-center justify-center gap-4 shrink-0"
  ...
  >
    <template v-if="decision === 'undecided'">
      <Button
        variant="outline"
        size="lg"
        class="min-w-[200px] h-12 text-base border-l-[3px] border-l-destructive hover:bg-destructive/5 hover:border-l-destructive"
      ...
      >
        ...
        <span>Exclude</span>
        <span
          v-for="reviewer in excludeReviewers"
          :key="reviewer.role"
          class="ml-1 text-[11px] px-1.5 py-0.5 rounded border border-border/60 text-muted-foreground font-normal"
          :data-testid="`${testIdPrefix}-btn-exclude-reviewer-${reviewer.role}`"
        >
      ...
      <Button
        variant="outline"
        size="lg"
        class="min-w-[200px] h-12 text-base border-l-[3px] border-l-green-600 hover:bg-green-600/5 hover:border-l-green-600"
      ...
      >
        ...
        <span>Include</span>
        <span
          v-for="reviewer in includeReviewers"
          :key="reviewer.role"
          class="ml-1 text-[11px] px-1.5 py-0.5 rounded border border-border/60 text-muted-foreground font-normal"
          :data-testid="`${testIdPrefix}-btn-include-reviewer-${reviewer.role}`"
        >
          @{{ reviewer.github_login }}
        </span>
        <span
